...
class Match implements Taskable {
    ...
    // tactical pause variables
    private $tacticalPauses = array("TERRORIST" => 0, "CT" => 0);
    private $isTacticalPause = False;
    private $TacticalPauseTeam = "";
    ...
    public function __construct($match_id, $server_ip, $rcon) {
        ...
        try {
            ...
            // adding sv_pausable after rcon
            $this->rcon->send("sv_pausable 1");
        } catch (\Exception $ex) {
            $this->needDel = true;
...
//          $this->addLog($message->userName . " (" . $message->userTeam . ") purchased " . $message->object);
    ...
    private function processSay(\eBot\Message\Type\Say $message) {
        ...
        if (preg_match('/\!map (?<mapname>.*)/i', $text, $preg)) {
            ...
            if ($this->pluginCsay) {
                $this->say_player($message->userId, "commands available: !tacpause, !status, !stats, !morestats, !score, !ready, !notready, !stop, !restart (knife round), !stay, !switch");
            }
        ...
        // tactical pause scheduling
        } else if($this->isCommand($message, "tacpause")){
            $team = $message->getUserTeam();
            if($this->isMatchRound() && $this->enable && !$this->isTacticalPause) {
                if($this->tacticalPauses[$team] < 2){
                    $this->TacticalPauseTeam = $team;
                    $this->say($team . " a tactical pause will start next round.");
                    $this->isTacticalPause = True;
                    $this->tacticalPauses[$this->TacticalPauseTeam] += 1;
                }else{
                    $this->say($team . " you can only start 02 tactical pauses.");
                }
            }else {
                if(!$this->isMatchRound()){
                    $this->say($team . " you can request a tactical pause only when match is started.");
                }elseif($this->isTacticalPause){
                    $this->say($team . " you have already requested a tactical pause.");
                }
...
//        $this->addMatchLog($this->getColoredUserNameHTML($message->userName, $message->userTeam) . " assisted the kill of " . $this->getColoredUserNameHTML($message->killedUserName, $message->killedUserTeam));
    ...
    private function processRoundStart(\eBot\Message\Type\RoundStart $message) {
        ...
        // executing a tactical if it exists
        if($this->TacticalPauseTeam != ""){
            $this->say($this->TacticalPauseTeam . " 60 seconds tactical pause started.");
            $this->rcon->send("pause");
            sleep(60);
            $this->rcon->send("unpause");
            $this->isTacticalPause = False;
            $this->say($this->TacticalPauseTeam . " tactical pause ended.");
            $this->TacticalPauseTeam = "";
        }
    }
